...
async function main() {
  try {
    // Parse the raw text into structured objects
    ...
    // 1. Get the most recently created presentation
    ...
    if (fetchError || !decks || decks.length === 0) {
      console.error("Could not find a presentation to add questions to.", fetchError);
      return;
    }

    const deckId = decks[0].id;
    console.log(`Adding to presentation ID: ${deckId}`);
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      if (qError || !createdQuestion) {
        console.error(`Failed to insert question ${i + 2}:`, qError);
        continue;
      }
      ...
      if (optError) {
        console.error(`Failed to insert options for question ${i + 2}:`, optError);
      } else {
        console.log(`Inserted question ${i + 2} successfully.`);
      }
    }
